...
class CNN_GRU(nn.Module):
  def __init__(self):
    ...
    self.gru = nn.GRU(input_size=512, hidden_size=32, num_layers=3)
...
criterion = nn.L1Loss()
optimizer = optim.SGD(cnngru.parameters(), lr=0.01)
...
for epoch in range(num_epochs):
  for i in range(len(train["aps_frame"])):
    ...
    output = cnngru(queue_tens.to(device))
    optimizer.zero_grad()
  ...
  if ((epoch +1)%50) == 0:
    torch.save(cnngru.state_dict(), f"./weights{epoch + 1}.pt")
...
plt.legend()
plt.savefig("./acc_cnngru_300.png")
plt.figure()
...
plt.legend()
plt.savefig("./brk_cnngru_300.png")
plt.figure()
...
plt.legend()
plt.savefig("./str_cnngru_300.png")
plt.figure()

predictions_tensor = torch.tensor(predictions, dtype=torch.float32)
torch.save(predictions_tensor, "./predictions_cnngru_300.pt")
